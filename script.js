...
async function carregarLista(nomeLista) {
  ...
  if (snap.exists) {
    ...
    if (data && Array.isArray(data.nomes) && data.nomes.length > 0) {
      dados = data.nomes;
    } else {
      console.warn(`⚠️ Lista ${nomeLista} vazia — recriando com nomes padrão.`);
    ...
    console.warn(`⚠️ Lista ${nomeLista} inexistente — criando com nomes padrão.`);
    dados = NOMES_INICIAIS.map(n => ({
      nome: n,
      cols: [false, false, false],
      data: new Date().toISOString()
